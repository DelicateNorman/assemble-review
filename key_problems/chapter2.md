...
## 目录
1. [2.1 已知DS = 2000H, BX = 0100H, SI = 0002H，存储单元[20100H]～[20103H]中依次存放12H、34H、56H、78H，[21200H]～[21203H]中依次存放2AH、4CH、B7H、65H，说明下列每条指令执行后AX寄存器的内容](#21-已知ds--2000h-bx--0100h-si--0002h存储单元20100h20103h中依次存放12h34h56h78h21200h21203h中依次存放2ah4chb7h65h说明下列每条指令执行后ax寄存器的内容)
2. [2.4 什么是堆栈？它的工作原则是什么？它的基本操作有哪两个？对应哪两种指令？](#24-什么是堆栈它的工作原则是什么它的基本操作有哪两个对应哪两种指令)
3. [2.6 给出下列各条指令执行后AL值，以及CF、ZF、SF、OF和PF的状态](#26-给出下列各条指令执行后al值以及cfzfsfof和pf的状态)
4. [2.7 设X、Y、Z均为双字数据，分别存放在地址为X、X+2、Y、Y+2、Z、Z+2的存储单元中，它们的运算结果存入W单元。阅读如下程序段，给出运算公式。](#27-设xyz均为双字数据分别存放在地址为xx2yy2zz2的存储单元中它们的运算结果存入w单元阅读如下程序段给出运算公式)
5. [2.8 分别用一条汇编语言指令完成如下功能](#28-分别用一条汇编语言指令完成如下功能)
6. [2.10 指出下列指令的错误](#210-指出下列指令的错误)
7. [2.23 有一个首地址为ARRAY的20个字的数组，说明下列程序段的功能](#223-有一个首地址为array的20个字的数组说明下列程序段的功能)
8. [2.24 按照下列要求，编写相应的程序段](#224-按照下列要求编写相应的程序段)
9. [2.26 参照图2-11，分析调用序列，画出每次调用及返回时的堆栈状态。其中CALL前是该指令所在的逻辑地址。另外，段内直接调用指令的机器代码的字节数为3，段间直接调用指令则为5字节。](#226-参照图2-11分析调用序列画出每次调用及返回时的堆栈状态其中call前是该指令所在的逻辑地址另外段内直接调用指令的机器代码的字节数为3段间直接调用指令则为5字节)
...
- **SF**：1（最高位为1）
- **OF**：0（0减1不会溢出）
- **PF**：1（1111 1111中有8个1，为偶数，但实际算奇数，重新计算：8个1为偶数）
...
**解析**：
- **指令**：ADD WORD PTR [0520H], 3412H
- **功能**：[0520H] ← [0520H] + 3412H
- **寻址方式**：直接寻址
- **操作数**：目的操作数[0520H]（存储器），源操作数3412H（立即数）
- **内存地址**：DS × 16 + 0520H
- **数据类型**：字操作（16位）
- **类型说明符**：必须使用`WORD PTR`明确指定操作数为字类型，否则汇编器无法确定是字节还是字操作
...
| (4) | add word ptr [0520h], 3412h | [0520H] ← [0520H] + 3412H | 直接寻址 | [0520H] | 3412H |
...
**程序段：**
```asm
mov dl, string        ; DX ← STRING[0]（第1个字符）
